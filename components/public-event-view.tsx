...
function FormFieldPreview({ field, primary, accent }: { field: FormField; primary: string; accent: string }) {
  ...
  return (
    <div>
      ...
      {field.type === "short_text" && <div style={inputStyle}>Short answer</div>}
      {field.type === "long_text" && <div style={{ ...inputStyle, minHeight: "4.5rem" }}>Long answer</div>}
      {field.type === "dropdown" && (
...
function ParticipantsGauge({ registered, max, primary, accent }: { registered: number; max: number; primary: string; accent: string }) {
  ...
  const R = 54, cx = 64, cy = 64, startAngle = -210, totalAngle = 240;
  ...
  const fillPath = pct > 0 ? `M ${arcStart.x} ${arcStart.y} A ${R} ${R} 0 ${sweepAngle > 180 ? 1 : 0} 1 ${fillEnd.x} ${fillEnd.y}` : "";
  void accent;
  return (
    <div className="mx-auto mt-4 mb-2" style={{ maxWidth: "200px" }}>
      <div className="relative flex items-center justify-center">
        <svg width="128" height="100" viewBox="0 0 128 100">
          ...
          {fillPath && <path d={fillPath} fill="none" stroke={gaugeStroke} strokeWidth="10" strokeLinecap="round" />}
          <text x={cx} y={cy + 4} textAnchor="middle" fill="#ffffff" fontSize="18" fontWeight="bold">{Math.round(pct)}%</text>
          <text x={cx} y={cy + 18} textAnchor="middle" fill="#ffffff80" fontSize="7">filled</text>
        </svg>
      </div>
      <div className="text-center -mt-2">
        {isFull
          ? <p className="text-xs font-semibold" style={{ color: "#ff6568" }}>Fully booked</p>
          : <p className="text-xs" style={{ color: "#ffffff80" }}><span className="font-semibold text-white">{spotsLeft}</span> spot{spotsLeft !== 1 ? "s" : ""} left of {max}</p>
        }
      </div>
    </div>
  );
  ...
  return (
    <div className="text-center rounded-2xl p-6" style={{ background: `${primary}15`, border: `1px solid ${primary}30` }}>
      ...
      <div className="flex items-end justify-center gap-2 sm:gap-4">
        {[{ val: days, label: "days" }, { val: hrs, label: "hrs" }, { val: mins, label: "min" }, { val: secs, label: "sec" }].map(({ val, label }, i, arr) => (
          <div key={label} className="flex items-end gap-2 sm:gap-4">
            ...
            {i < arr.length - 1 && <p className="text-2xl font-bold mb-5" style={{ color: "#ffffff40" }}>:</p>}
          </div>
        ))}
      </div>
